...
HEALTH_CHECK_HOST="${HEALTH_CHECK_HOST:-10.16.52.102}"  # Internal host for stale tunnel detection (TCP, not ping)
HEALTH_CHECK_PORT="${HEALTH_CHECK_PORT:-22}"            # SSH port — reliable for TCP connect checks
SUDO_KEEPALIVE_PID=""
...
monitor_vpn_connection() {
    ...
    while true; do
        ...
        # The process can stay alive and ppp0 can exist while the tunnel
        # is actually dead (stale TLS session). If 3 consecutive checks
        # fail, kill the process to trigger the reconnect logic above.
        # Uses TCP connect (nc) since many internal hosts block ICMP/ping.
        local _hc_host="${HEALTH_CHECK_HOST:-10.16.52.102}"
        local _hc_port="${HEALTH_CHECK_PORT:-22}"
        local _hc_fail=0
        for _attempt in 1 2 3; do
            if nc -z -w 3 "$_hc_host" "$_hc_port" 2>/dev/null; then
                _hc_fail=0
                break
            fi
            _hc_fail=$_attempt
            log_warn "VPN health check: $_hc_host:$_hc_port unreachable ($_attempt/3)"
            [ "$_attempt" -lt 3 ] && sleep 5
        done
        if [ "$_hc_fail" -ge 3 ]; then
            log_error "VPN health check: 3 consecutive checks failed — tunnel is stale"
            # Remove split DNS BEFORE killing — the resolver files point
            # *.bayportfinance.com to internal DNS which is unreachable,
            # and openfortivpn needs to resolve the gateway hostname to reconnect.
            remove_split_dns
            log_info "Killing stale VPN process (PID: $vpn_pid) to trigger reconnect..."
            kill "$vpn_pid" 2>/dev/null
            continue  # loop back to reconnect logic
        fi
...
connect_vpn() {
    ...
    # trap never fires and /etc/resolver/<domain> files point to internal
    # DNS servers that aren't reachable without the VPN — blocking reconnect.
    # Match by our comment marker rather than domain list (which isn't parsed yet).
    if [ -d /etc/resolver ]; then
        for f in /etc/resolver/*; do
            [ -f "$f" ] || continue
            if grep -q "Auto-generated by bayport-vpn" "$f" 2>/dev/null; then
                log_info "Removing stale resolver: $f"
                sudo rm -f "$f"
            fi
        done
    fi
